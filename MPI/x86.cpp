...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < sizeof(N_values) / sizeof(N_values[0]); ++i)
    {
        ...
        mpi_block();          // MPI块划分
        mpi_circle();         // MPI循环划分
        mpi_block_avx();      // MPI块划分+avx
        mpi_block_omp();      // MPI块划分+OpenMP
        mpi_block_neon_omp(); // MPI块划分+avx+OpenMP
        mpi_async();          // MPI块划分+非阻塞通信
        mpi_async_avx();      // MPI块划分+非阻塞通信+avx
        mpi_async_omp();      // MPI块划分+非阻塞通信+OpenMP
        mpi_async_avx_omp();  // MPI块划分+非阻塞通信+avx+OpenMP
    }
    MPI_Finalize();
    return 0;
}
