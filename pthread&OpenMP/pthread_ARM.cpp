...
int main()
{
    ...
    for (int i = 0; i < sizeof(N_values) / sizeof(N_values[0]); ++i) {
        ...
        cout << "neon优化耗时: " << duration << " ms" << endl;
        ...
        reset();
        start_time = high_resolution_clock::now();                             // 开始时间
        neon_dynamic();                                             // 执行neon优化动态线程高斯消去法
        end_time = high_resolution_clock::now();                               // 结束时间
        duration = duration_cast<milliseconds>(end_time - start_time).count(); // 计算时间差
        cout << "neon优化动态线程耗时: " << duration << " ms" << endl;

        reset();
        start_time = high_resolution_clock::now();                             // 开始时间
        normal_sem_static();                                        // 执行平凡算法静态线程+信号量同步高斯消去法
        end_time = high_resolution_clock::now();                               // 结束时间
        duration = duration_cast<milliseconds>(end_time - start_time).count(); // 计算时间差
        ...
        reset();
        start_time = high_resolution_clock::now();                             // 开始时间
        neon_sem_static();                                          // 执行neon静态线程+信号量同步高斯消去法
        end_time = high_resolution_clock::now();                               // 结束时间
        duration = duration_cast<milliseconds>(end_time - start_time).count(); // 计算时间差
        cout << "neon优化静态线程+信号量同步耗时: " << duration << " ms" << endl;

        reset();
        start_time = high_resolution_clock::now();                             // 开始时间
        normal_sem_tri();                                           // 执行平凡算法静态线程+信号量同步+三重循环全部纳入线程函数高斯消去法
        end_time = high_resolution_clock::now();                               // 结束时间
        duration = duration_cast<milliseconds>(end_time - start_time).count(); // 计算时间差
        ...
        reset();
        start_time = high_resolution_clock::now();                             // 开始时间
        neon_sem_triplecircle();                                    // 执行neon静态线程+信号量+三重循环全部纳入线程函数高斯消去法
        end_time = high_resolution_clock::now();                               // 结束时间
        duration = duration_cast<milliseconds>(end_time - start_time).count(); // 计算时间差
        cout << "neon优化静态线程+信号量同步+三重循环全部纳入线程函数耗时: " << duration << " ms" << endl;

        reset();
        start_time = high_resolution_clock::now();                             // 开始时间
        normal_barrier();                                           // 执行平凡算法静态线程+barrier同步高斯消去法
        end_time = high_resolution_clock::now();                               // 结束时间
        duration = duration_cast<milliseconds>(end_time - start_time).count(); // 计算时间差
        ...
        reset();
        start_time = high_resolution_clock::now();                             // 开始时间
        neon_barrier();                                             // 执行neon静态线程+barrier同步高斯消去法
        end_time = high_resolution_clock::now();                               // 结束时间
        duration = duration_cast<milliseconds>(end_time - start_time).count(); // 计算时间差
        cout << "neon优化静态barrier耗时: " << duration << " ms" << endl;
    }
    return 0;
}
